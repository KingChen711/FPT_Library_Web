...
export type FOV = {
  id: string
  f: EAdvancedFilterBorrowRequestField | null
...
const AdvancedSearchSection = ({ queries, setQueries }: Props) => {
  ...
  const selectedFields = useMemo(
    () =>
      queries
        .map((item) => item.f)
        .filter(Boolean) as EAdvancedFilterBorrowRequestField[],
    [queries]
  )
  ...
  const handleAddFov = () => {
    if (queries.length >= borrowRequestAdvancedFilters.length) return
...
type AdvancedSearchItemProps = {
  fov: FOV
  selectedFields: EAdvancedFilterBorrowRequestField[]
  onDeleteFov: () => void
...
export function AdvancedSearchItem({
  ...
  const type = useMemo(
    () =>
      fov.f
        ? (borrowRequestAdvancedFilters.find((a) => a.field === fov.f)!.type ??
          null)
        : null,
    [fov.f]
  )
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <Popover open={openSelectF} onOpenChange={setOpenSelectF}>
        <PopoverTrigger asChild>
          ...
          >
            {fov.f
              ? t(
                  borrowRequestAdvancedFilters.find((f) => f.field === fov.f)
                    ?.field
                )
              : t("Select")}
        ...
        <PopoverContent className="w-[200px] shrink-0 p-0" side="bottom">
          <Command>
            ...
            <CommandList className="max-h-[50dvh] overflow-y-auto">
              ...
              <CommandGroup>
                {borrowRequestAdvancedFilters.map((f) => {
                  ...
                  return (
                    <CommandItem
                      ...
                      onSelect={() => {
                        onChangeFov({
                          ...fov,
                          f: f.field as EAdvancedFilterBorrowRequestField,
                          o: defaultO[f.type],
                          v: defaultV[f.type],
                        })
      ...
      {fov.f === EAdvancedFilterBorrowRequestField.CATEGORY && (
        <Popover
