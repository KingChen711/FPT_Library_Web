...
function CoverImageField({
  ...
  const watchAuthorIds = form.watch(`libraryItem.authorIds`)

  const watchTitle = form.watch(`libraryItem.title`)
  const watchSubTitle = form.watch(`libraryItem.subTitle`)
  const watchGeneralNote = form.watch(`libraryItem.generalNote`)
  const watchPublisher = form.watch(`libraryItem.publisher`)
  const watchFile = form.watch(`libraryItem.file`)
  const watchValidImage = form.watch(`libraryItem.validImage`)
  const watchCheckedResult = form.watch(`libraryItem.checkedResult`)
  ...
  const onDrop = (acceptedFiles: File[]) => {
    if (acceptedFiles.length > 0) {
      ...
      form.setValue(`libraryItem.file`, file)
      const url = URL.createObjectURL(file)
      form.setValue(`libraryItem.coverImage`, url)
      form.clearErrors(`libraryItem.coverImage`)
    }
  }
  ...
  const handleCheckImage = () => {
    ...
    checkImage(formData, {
      onSuccess: (data) => {
        ...
        form.setValue(`libraryItem.checkedResult`, data[0])
        form.setValue(`libraryItem.validImage`, validImage)
        if (validImage)
          if (validImage) form.clearErrors(`libraryItem.coverImage`)
      },
      onError: () => {
        form.setValue(`libraryItem.checkedResult`, undefined)
        form.setValue(`libraryItem.validImage`, false)
        toast({
          title: locale === "vi" ? "Thất bại" : "Failed",
          description:
            locale === "vi"
  ...
  useEffect(() => {
    if (!mounted) return
    form.setValue(`libraryItem.validImage`, undefined)
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [
    form,
    watchTitle,
                ...
                >
                  ...
                  <Button
                    onClick={(e) => {
                      e.preventDefault()
                      field.onChange("")
                      form.setValue(`libraryItem.file`, undefined)
                    }}
                    variant="ghost"
                    size="icon"
