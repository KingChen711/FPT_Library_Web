...
import { deleteLibraryPackage } from "@/actions/library-packages/delete-library-package"
...
function DeletePackageDialog({
  ...
  const handleDeleteCategory = () => {
    startTransition(async () => {
      const res = await deleteLibraryPackage(libraryPackageId)
      if (res.isSuccess) {
        toast({
        ...
        router.push("/management/packages")
        return
      }
  ...
  return (
    <Dialog open={openDelete} onOpenChange={setOpenDelete}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle className="mb-1">{t("Delete package")}</DialogTitle>
          <DialogDescription>
            <div
