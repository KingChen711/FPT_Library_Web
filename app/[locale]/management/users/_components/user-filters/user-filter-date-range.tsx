import { useEffect, useState } from "react"
import { usePathname, useRouter, useSearchParams } from "next/navigation"
import { Trash } from "lucide-react"
import { useTranslations } from "next-intl"
import { DateRangePicker, type RangeKeyDict } from "react-date-range"

import { type EmployeeFilter } from "@/lib/validations/employee/employees-filter"
import { Input } from "@/components/ui/input"
import { Label } from "@/components/ui/label"

type EmployeeFilterDateRangeProps = {
  label: string
  enumField: EmployeeFilter
}

const EmployeeFilterDateRange = ({
  label,
  enumField,
}: EmployeeFilterDateRangeProps) => {
  const router = useRouter()
  const pathname = usePathname()
  const tGeneralManagement = useTranslations("GeneralManagement")
  const searchParams = useSearchParams()
  const [isOpen, setIsOpen] = useState(false)
  const [state, setState] = useState<(Date | null)[]>([])

  // Function to format date as 'YYYY-MM-DD'
  const formatDate = (date: Date | null) => {
    if (!date) return ""
    const year = date.getFullYear()
    const month = (date.getMonth() + 1).toString().padStart(2, "0")
    const day = date.getDate().toString().padStart(2, "0")
    return `${year}-${month}-${day}`
  }

  useEffect(() => {
    const params = new URLSearchParams(searchParams)
    params.delete(enumField)
    if (state[0]) {
      params.append(enumField, formatDate(state[0]))
    }
    if (state[1]) {
      params.append(enumField, formatDate(state[1]))
    }
    const newUrl = `${pathname}?${params.toString()}`
    // Only replace the URL if the searchParams have actually changed
    if (newUrl !== window.location.href) {
      router.replace(newUrl)
    }
  }, [state, enumField, pathname, router, searchParams])

  const handleReset = () => {
    setState([])
    const params = new URLSearchParams(searchParams)
    params.delete(enumField)
    router.replace(`${pathname}?${params.toString()}`)
  }

  return (
    <div className="flex items-center gap-4">
      <Label className="w-1/4 text-nowrap">{label}</Label>

      <div className="relative flex-1">
        <Input
          readOnly
          value={
            state[0]
              ? `${formatDate(state[0])}${
                  state[1] ? ` - ${formatDate(state[1])}` : ""
                }`
              : tGeneralManagement("placeholder.date range")
          }
          className="cursor-pointer"
          onClick={() => setIsOpen(!isOpen)} // Toggle visibility
        />

        {isOpen && (
          <div className="absolute z-10 mt-2 rounded border bg-primary-foreground shadow">
            <DateRangePicker
              editableDateInputs={true}
              ranges={[
                {
                  key: "selection",
                  startDate: state[0] ?? undefined, // Allow `undefined` as a valid type
                  endDate: state[1] ?? undefined, // Allow `undefined` as a valid type
                },
              ]}
              moveRangeOnFirstSelection={false}
              onChange={(ranges: RangeKeyDict) => {
                const selection = ranges.selection
                if (selection) {
                  const { startDate, endDate } = selection
                  setState([startDate ?? null, endDate ?? null])
                }
              }}
            />
            <div className="flex justify-end p-2">
              <button
                className="rounded bg-primary px-4 py-2 text-sm text-primary-foreground"
                onClick={() => setIsOpen(false)}
              >
                {tGeneralManagement("btn.close")}
              </button>
            </div>
          </div>
        )}
      </div>

      <Trash
        size={24}
        color="red"
        className="cursor-pointer"
        onClick={handleReset}
      />
    </div>
  )
}

export default EmployeeFilterDateRange
