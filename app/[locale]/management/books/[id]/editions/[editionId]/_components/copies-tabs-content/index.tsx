...
type Props = {
  copies: BookEditionCopy[]
  bookId: number
  editionId: number
}

function CopiesTabsContent({ copies, bookId, editionId }: Props) {
    ...
    <TabsContent value="copies">
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-4">
          ...
          <AddCopiesDialog bookId={bookId} editionId={editionId} />
        </div>
      </div>
                  ...
                  <TableRow key={copy.bookEditionCopyId}>
                    <TableCell>
                      <Checkbox
                        ...
                        onCheckedChange={() => {
                          if (
                            selectedCopyIds.includes(copy.bookEditionCopyId)
                          ) {
                            setSelectedCopyIds((prev) =>
                              prev.filter((id) => id !== copy.bookEditionCopyId)
                            )
                          } else {
                            setSelectedCopyIds((prev) => [
                              ...prev,
                    ...
                    <TableCell>
                      <div className="flex justify-center">
                        <CopyDropdown bookId={bookId} copy={copy} />
                      </div>
                    </TableCell>
                  </TableRow>
                ))}
