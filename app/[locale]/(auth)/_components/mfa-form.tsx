...
function MfaForm({ email }: Props) {
  const t = useTranslations("ResetPasswordPage")
  const [pendingMfa, startMfa] = useTransition()
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        ...
        />
        <Button disabled={pendingMfa} type="submit" className="w-full">
          {t("Continue")}{" "}
          {pendingMfa && <Loader2 className="size-4 animate-spin" />}
        </Button>
