...
export async function createBorrowRecordFineTransaction(
...
> {
  ...
  try {
    const { data, message } = await http.post<{
      ...
      `/api/payment/transactions/borrows/records/${borrowRecordId}`,
      {},
      {
    ...
    revalidatePath(`/me/account/borrow/record/${borrowRecordId}`)
